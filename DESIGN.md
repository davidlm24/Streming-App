...
<!-- Registrado a partir do código entregue (casca do app, Painel, Canais, Webinars, Configurações, rodapé). Onde o mock mockups/design-system.html e o código divergem, vale o código. Valores em hex: os do tema escuro, que é o padrão; a troca por tema está em Colors. -->
...
- Vermelho = ar. Não entra em gráfico, em alerta nem em decoração.
...
- Um único momento de movimento: o chip de um canal recém-conectado entra deslizando uma vez.
...
### Primary
- **Azul da Marca Profundo** (`brand-deep`): o preenchimento do botão primário (`.btn`). É a única área cromática da casca. Texto branco sobre ele. Há um botão primário por tela: "Entrar no estúdio" (tamanho `lg`) no Painel, "Conectar canal" em Canais e "Agendar webinar" em Webinars.
...
| `--panel` | esqueleto de carregamento, trilho do switch desligado, item de menu em hover, destino atual no menu móvel | navy-14 | navy-92 | navy-10 |
| `--raise` | hover de botão fantasma/ícone/chip, segmento ativo, fundo do menu | navy-18 | navy-100 | navy-14 |
...
| `--ink-hi` | títulos, nomes, destino atual, switch ligado | navy-96 | navy-6 | navy-96 |
| `--ink` | corpo; a hora em mono | navy-89 | navy-18 | navy-89 |
| `--ink-lo` | texto secundário, descrições, ação de texto em repouso, destinos não atuais | navy-80 | navy-28 | navy-70 |
...
### Named Rules
**Regra da Voz Única.** Numa tela da casca, só o botão da ação da tela tem cor. Ligado, selecionado, atual e pronto são ditos pela rampa (`--ink-hi`, `--raise`), nunca pelo azul. Switch ligado é tinta cheia; segmento ativo sobe um degrau; destino atual ganha um traço de 2px em `--ink-hi`.

**Regra do Ar.** O carmim é o sinal de "no ar". Fora do ar, aparece só onde algo é apagado ou encerrado. Não entra em pendência, em aviso, em gráfico nem em ícone de marca. Uma pendência de canal ("sem chave") fica na tinta do nome, com ícone de alerta.

**Regra da Marca Monocromática.** Ícones de plataforma (YouTube, Twitch…) são Lucide em `currentColor`, traço 1,75, nunca nas cores do dono da marca.
...
- **Title** (600, `text-base`): título de seção (`h2` de `SecaoDePagina`, "Canais", "Próximas lives").
- **Body** (400 ou 500, `text-sm`): descrições (`max-w-prose`), nomes em linhas de lista (500), rótulos de botão e de chip, ações de texto.
- **Label** (400, `text-xs`): a linha de metadados abaixo de um nome (plataforma · estado, horário · canais), rodapé e aviso de teste no cabeçalho.
...
Cantos generosos e consistentes, e uma geometria que distingue controle de estrutura. Os controles são arredondados (botão 16px, botão denso 10px, botão de ícone 16px, menu 16px com itens de 12px, segmentado 16px por fora e 10px por dentro, modal 20px). Chips e switches são pílulas completas. A estrutura (linhas, seções) é reta e sem raio. O anel de foco tem 4px de raio.
...
- **Primary:** preenchimento em `brand-deep` com texto branco, 9px × 18px, 14px/600, ícone a 8px do rótulo. Um por tela, no máximo.
...
A voz de tudo o que é secundário: sem caixa, sem cor. Fica em `--ink-lo` e passa a `--ink-hi` com sublinhado no hover, em `text-sm` no corpo e `text-xs` no rodapé e no cabeçalho. A variante `sublinhada` serve para o link no meio de uma frase: tinta alta, sublinhado em `--line-ctl`, afastado 4px. Um ícone opcional à esquerda (ex.: `+` em "Conectar canal").
...
- **State:** à direita, um ✓ para pronto, ou um ícone de alerta com a pendência em palavra ("sem chave", "sem servidor", "desligado", "não conectado"), sempre na tinta do nome. O `aria-label` carrega a frase inteira ("YouTube Principal: pronto"). Clicar numa pendência leva direto a consertá-la.
...
### Inputs / Fields
- **Style:** campos nativos seguem os tokens globalmente: fundo `--well`, texto `--ink-hi`, borda `--line-ctl`, placeholder `--ink-dim`.
...
- **Error / Disabled:** desabilitado a 45 %. A casca redesenhada não tem ainda um estado de erro de campo próprio.
...
- **Don't** usar o carmim fora do ar e das ações que apagam ou encerram: nada de vermelho em pendência, aviso, gráfico ou decoração.
...
- **Don't** colorir ícones de plataforma com as cores das marcas.
...
Estas partes ainda carregam o visual herdado do Google AI Studio e **não** são referência: o estúdio (`Header.tsx`, `LeftSidebar`, o palco), o interior do `AddChannelsModal` e do `VideoQualityPanel` (que abre dentro de Configurações), as páginas de cobrança, cadastro, admin e super-admin, o site público e os preços, e o modal de criar webinar em `App.tsx`. Os sinais típicos são tamanhos em px avulsos, `slate`/`gray`/`blue-500` crus, rótulos em caixa alta e sombras de brilho. A catraca mede essa dívida (na linha de base atual: 1.018 tamanhos avulsos, 469 `<button>` crus fora das primitivas, 92 usos estruturais de `slate`/`gray`, 119 `focus:outline-none`). Ao tocar numa dessas telas, migre para as primitivas e os papéis deste documento, sem copiar o que está lá.

Dentro das superfícies redesenhadas, alguns desvios também ficam fora do sistema: o gatilho "⋯" do `Menu` (32px) e o avatar da conta (32px) estão abaixo do alvo de 44px, o chip tem 36px, o cabeçalho usa `z-30` em vez de `--z-sticky`, e o polegar da barra de rolagem fica azul no hover. A linha "ONLINE STUDIO" do logo (9px, mono, caixa alta) faz parte da marca e não é modelo de rótulo.
